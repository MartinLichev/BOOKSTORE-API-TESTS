        ...
        script {
          docker.withTool('DockerCLI') {
            docker.image('maven:3.9.9-eclipse-temurin-17').inside('-v $HOME/.m2:/root/.m2') {
              sh '''
                mvn -q -ntp -B -P ${MAVEN_PROFILE} clean test                   -DbaseUrl=${BASE_URL}                   -DapiBasePath=${API_BASE_PATH}                   -DuseVersionedMediaType=${USE_VERSIONED_MEDIA_TYPE}                   -DversionedMediaType="${VERSIONED_MEDIA_TYPE}"                   -DlogOnFailure=true
              '''
            }
          }
        }
