pipeline {
  ...
  environment {
    ...
    USE_VERSIONED_MEDIA_TYPE = "${params.USE_VERSIONED_MEDIA_TYPE}"
    VERSIONED_MEDIA_TYPE = "${params.VERSIONED_MEDIA_TYPE}"
  }
        ...
        script {
          docker.withTool('DockerCLI') {
            docker.image('maven:3.9.9-eclipse-temurin-17').inside('-v $HOME/.m2:/root/.m2') {
              sh '''
                mvn -q -ntp -B clean test                   -DbaseUrl=$BASE_URL                   -DapiBasePath=$API_BASE_PATH                   -DuseVersionedMediaType=$USE_VERSIONED_MEDIA_TYPE                   -DversionedMediaType="$VERSIONED_MEDIA_TYPE"
              '''
            }
          }
        }
  ...
  post {
    always {
      ...
      script {
        try { allure includeProperties: false, jdk: '', results: [[path: 'target/allure-results']] }
        catch (e) { echo 'Allure not configured; skipping.' }
      }
    }
  }
}
